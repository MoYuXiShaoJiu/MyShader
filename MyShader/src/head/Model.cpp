...
void MyModel::ProcessNode(aiNode* node, const aiScene* scene)//µÝ¹éµÄ´¦Àínode
{
...
Mesh MyModel::ProcessMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    // 2. specular maps
    std::vector<Texture> specularMaps = loadMaterialTextures(material, aiTextureType_SPECULAR, TexType::spec);
...
std::vector<Texture> MyModel::loadMaterialTextures(aiMaterial* mat, aiTextureType type, TexType typeName)
{
    std::vector<Texture> textures;
